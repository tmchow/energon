...
- `hub-cleanup-matching` is `#catalog-select-matching`, under the list next to Select visible. The target is the current catalog filters (`q`, `scope`, `kind`, `expires`, `expires_before`, `updated_before`, `min_size`), not the loaded page of ids. Matching does not send `last_read_before`. Unchecking a row after matching turns the target into the remaining visible ids.
...
- `hub-cleanup-drift` is `409 cleanup_drift` when the selection (or action) changed between preview and execute. The dialog says the selection changed and reloads the facts. Review the new count and confirm again.
