...
- **Proof.** Save whoami JSON, bad-ttl JSON, expired JSON, the bulk preview and drift JSON, account/data excerpt (redact nothing in evidence — this is a disposable local token), and screenshots of `#new-token`, the greyed `verify-expired` row, the `Stale` badge, and `#bulk-dlg` on `/tokens`.
