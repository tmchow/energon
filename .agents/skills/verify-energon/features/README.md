...
- [Cleanup](./cleanup.md) covers the list filters and sorts (`expires`, `expires_before`, `expires_within=24h|7d`, `updated_before`, `min_size`, `sort=size|age`), the `POST /v1/cleanup` dry run, confirm-to-execute, drift, skip reasons, the 100-object cap, and body validation.
