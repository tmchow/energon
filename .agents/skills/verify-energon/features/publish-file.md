...
- **Extra (file-markdown-html) — Rendered Markdown.** `GET` the public `.md` URL with `Accept: text/html`. Status `200`. Body contains `class="en-md"` and the rendered heading, `color-scheme:light dark` (spaces optional), and does not contain `class="en-brand"`, `class="en-top`, `class="en-card`, or `>Raw<`. `GET` the same URL without that Accept header (or with `?raw=1`) is still markdown source. Drive when Markdown.svelte or markdown page chrome changes.
