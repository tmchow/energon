...
- **Hub preview.** Open `/admin`, fill `#admin-owner` with `$OWNER_HANDLE`, `#admin-q` with `vadmin-old`, set expiry filter to Never expires, choose Preview. `#admin-sample` names `vadmin-old.md` and the owner handle; its `Last read` cell reads `No recorded read`. Save a screenshot.
- **Expire soon only for own work.** With `#admin-owner` empty or `$OWNER_HANDLE`, `#admin-action` has no `Expire soon` button. Type the doctor `HANDLE` (`dev` for the default identity) into `#admin-owner`: `Expire soon` appears. Replace it with `$OWNER_HANDLE`: it disappears and `Set expiry` is pressed again. Save both states.
