...
Hub catalog lets a signed-in human see sites and files they created or last wrote, search slugs and filenames, filter created vs edited, never-expiring / 24h / 7d / oldest / largest / last-read, open a public URL, change expiration, and delete an object after typing its name.
...
- **Default — Merged paging.** `POST $ORIGIN/v1/sites` `{"slug":"vpaging-site"}` (`201`). `POST $ORIGIN/v1/files` with Bearer `$TOKEN`, `X-Filename: vpaging-file.md`, `content-type: text/plain`, body `note` (`201`). `GET $ORIGIN/account/data?q=vpaging&sort=name&limit=1` returns one item with `kind` and a `cursor`; the same URL plus `&cursor=<that>` returns the next item of the other kind and no repeat. `GET $ORIGIN/account/data?q=vpaging&kind=sites` returns only `kind: "site"` items.
...
- `Load more` appears only when a cursor is present. Do not treat a short list as a pagination bug. The merged hub list pages with one `cursor` on `/account/data`. `sites_cursor` / `files_cursor` still alias `GET /v1/sites` and `GET /v1/files` when `cursor` is absent; they do not page the hub union.
