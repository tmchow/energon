...
- **Default — Merged paging.** `POST $ORIGIN/v1/files` with Bearer `$TOKEN`, `X-Filename: verify-note.md`, `content-type: text/plain`, body `note` (`201`). `GET $ORIGIN/account/data?q=verify&sort=name&limit=1` returns one item with `kind` and a `cursor`; the same URL plus `&cursor=<that>` returns the next item of the other kind and no repeat. `GET $ORIGIN/account/data?q=verify&kind=sites` returns only `kind: "site"` items. Do not delete `verify-note.md`.
