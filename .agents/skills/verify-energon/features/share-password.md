...
- **Clear.** Hub: open Link access, set `#pw-dlg-share-door` to Off, `Save`. Lock disappears. Public GET without header is 200 and contains `gated-ok`. API: `PATCH /v1/sites/verify-gated` with `{"password":""}` also clears.
