...
Operators listed on `ADMIN_EMAILS` see when this build is behind the latest public GitHub Release. The hub shows a warn **Update** badge on Admin. `/admin` explains the newer tag above Health. There is no deploy button. Dismiss is this browser only.
...
- `admin-upstream-update` is a lower baked semver than latest. Hub Admin links to `/admin#admin-update` with a warn badge that says `Update`. `#admin-update` sits above `#admin-health`, is not charged, lists Operator bullets, and has Read the release, How to update, and `#admin-update-dismiss`.
...
- Operator, Admin: open `/admin`. Current: version in the kicker. Update: quiet card above Health. Failed or unknown: one muted line. Dismiss is on the card.
...
- Launch with `ENERGON_VERIFY_EMAIL` set (or the default local identity) and `ENERGON_VERIFY_VARS="ADMIN_EMAILS:$EMAIL UPSTREAM_RELEASE_JSON:{\"tag_name\":\"v1.1.0\",\"html_url\":\"https://example.test/r/v1.1.0\",\"published_at\":\"2026-09-14T00:00:00.000Z\",\"body\":\"## Operator\\n- Restart after deploy\"}"` so the doctor identity is an operator and Admin does not call the network. `$EMAIL` must be the address `GET /account/data` reports (`.dev.vars` `DEV_ACCESS_EMAIL` if set, else `dev@example.com`). The JSON value must contain no spaces; launch splits `ENERGON_VERIFY_VARS` on whitespace.
