...
- **Default — Operator Admin HTML.** `GET $ORIGIN/admin` as the doctor is `200`. Contains `id="admin-update"`, `1.1.0 is available`, `1.1.0 was released`, `id="admin-update-dismiss"`, `Read the release`, `How to update`, and `This Energon ·` plus this build's `version.txt`. `#admin-update` appears before `#admin-health`. No `id="admin-update-operator"`. Bootstrap `upstream.status` is `update`. The update card class is `en-card en-admin-card` without `en-card--charged`. The date is the newer tag's `published_at`, not this build.
