...
Do not replace `wrangler.toml`, `instance-skill.json`, generated `plugins/`, or marketplace catalogs with upstream placeholders. Preserve deliberate code customizations when resolving conflicts. Regenerate the plugin from the resolved sources. Run the local checks in INSTALL.md **Commit and deploy** and affected user paths through verify-energon before deciding how to land.
