...
import { identityFromEnv, installLine, skillsAddCommand, REPO_MARKETPLACE, REPO_SKILL } from "../../src/instance";
...
describe("identityFromEnv", () => {
  it("this repo advertises the placeholder energon skill", () => {
    ...
    expect(skillsAddCommand(id)).toBe("npx skills add tmchow/energon --skill energon -g");
  });
  ...
  it("a private Energon advertises its own rendered skill and hostname", () => {
    ...
    expect(skillsAddCommand(id)).toBe("npx skills add cybertron/energon --skill cybertron-energon -g");
