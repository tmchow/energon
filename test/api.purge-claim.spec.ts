...
describe("TTL purge claims", () => {
  ...
  it("does not write after expiry claims a row between its snapshot and write claim", async () => {
    ...
    db.prepare = ((sql: string) => {
      ...
      if (snapshotPaused || !sql.includes("SELECT id, handle, filename, size, expires_at, created_by, last_written_by, updated_at, write_policy, owner_id, password_hash FROM loose_files WHERE id = ?")) {
        return statement;
      }
      snapshotPaused = true;
