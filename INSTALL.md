...
Detect whether automatic updates are on by a **successful** read of `ENABLE_PRODUCTION_DEPLOY` (`gh variable get`). `true` plus the Cloudflare Actions secrets means merging to `main` migrates D1 and deploys the Worker — ask before doing that. A successful read that is empty or not `true` means updating the fork does not change the live Worker; land the release, then ask if they want to deploy this release with laptop Wrangler. A failed get (404 or auth) is unclear — GitHub uses 404 for both unset and unread variables — so do not land on `main` without asking. Turning [automatic updates](#after-the-first-deploy-optional-automatic-updates) on after `main` already has the commit does not deploy that commit. Before any remote migrate, record a D1 Time Travel bookmark (`npx wrangler d1 time-travel info` with this fork's database name). Do not create, delete, empty, or rebind the live D1 or R2. Do not use GitHub “Sync fork”. A data backup with no upgrade is [`.agents/skills/backup-this-energon/SKILL.md`](.agents/skills/backup-this-energon/SKILL.md).
