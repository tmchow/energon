<script lang="ts">
  ...
  let { kind, items, cursor, busy, allowUnlimited, onMore, onPassword, onDelete, onLoadMore }:
...
{#snippet actions(item: CatalogItem)}
  <div class="en-row-actions">
    ...
    <IconButton icon="lock" label={item.password_protected ? 'Change or remove password' : 'Set password'} on={item.password_protected} extra onclick={() => onPassword(item)} />
