<script lang="ts">
  ...
  let { tone = 'ok', password, writePassword, children, action, onDismiss, className = '' }:
    { tone?: 'ok' | 'err' | 'warn'; password?: string; writePassword?: string; children?: Snippet; action?: Snippet; onDismiss?: () => void; className?: string } = $props();
</script>
<div class="en-flash en-flash--{tone} {className}" class:en-flash--dismissible={!!onDismiss} role={tone === 'err' ? 'alert' : 'status'}>
  <div class="en-flash-body">
    ...
    {#if action}<div class="en-flash-actions">{@render action()}</div>{/if}
  </div>
