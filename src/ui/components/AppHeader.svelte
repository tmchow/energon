...
<header class="en-top" class:en-top--public={publicPage}><div class="en-top-inner">
  ...
  <div class="en-top-end">
    {#if !publicPage}<nav class="en-nav" aria-label="Pages">{#each links as [id, href, label] (id)}<a {href} class:on={active === id} aria-current={active === id ? 'page' : undefined}>{label}{#if id === 'admin' && showUpdate}<Badge tone="warn">Update</Badge>{/if}</a>{/each}</nav>{/if}
