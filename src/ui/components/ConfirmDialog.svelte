<script lang="ts">
  ...
  let { open = $bindable(false), title, message, label, initial = '', match, noun, action, danger = false, busy = false, onConfirm, id = 'dlg', error = '' }:
    { open?: boolean; title: string; message: string; label?: string; initial?: string; match?: string; noun?: string;
...
<Dialog {id} bind:open {title} {message} dismissible={!busy}>
  ...
  <form onsubmit={submit}>
    ...
    {#if match !== undefined}<p class="en-note">Type the exact {noun ?? (action === 'Revoke' ? 'label' : 'name')}.</p>{/if}
