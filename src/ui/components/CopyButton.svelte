<script lang="ts">
  ...
  let { text, label = 'Copy', iconOnly = false, id, size = 'md' }: { text: string; label?: string; iconOnly?: boolean; id?: string; size?: 'sm' | 'md' } = $props();
...
{#if iconOnly}<IconButton {id} {size} icon={copied ? 'check' : 'clipboard'} label={copied ? 'Copied' : label} onclick={copy} />
