...
<main class="en-wrap">
  <PageTitle title="Agent tokens" wide>
    <p class="en-lede">Let an agent publish, read, reference, or update work on your behalf. Configure it with <code>{data.token_env}</code>; updates follow each file or site's write policy. The secret is shown once when you mint. The list shows the last four characters.</p>
    <p class="en-lede">A valid token can read work on this instance, including password-protected links. Token expiration stops agent access through that token; it does not expire published links. New here? <a href="/setup">Connect your agent</a>.</p>
  </PageTitle>
  <div id="messages">{#if error}<Flash tone="err">{error}</Flash>{/if}</div>
  <Card title="Mint" tight className="en-tokens-card">
    <div class="en-card-body">
      <form id="mint" class="en-mint" onsubmit={mint}>
        <Field label="Label" htmlFor="mint-label"><Input id="mint-label" name="label" bind:value={label} placeholder="laptop" required maxlength={64} disabled={busy} /></Field>
    ...
    <div id="tokens">
      ...
      {:else}<EmptyState title="No active tokens">Mint one with a label like laptop or ci.</EmptyState>{/if}
    </div>
  </Card>
</main>
