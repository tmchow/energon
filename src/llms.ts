...
- [Admin](${origin}/admin): operators listed on \`ADMIN_EMAILS\`. Retire old work across accounts; every action is recorded. Health shows quota used versus the cap. Signed-in humans only.
...
- An admin token is opt-in at mint on /tokens by someone on \`ADMIN_EMAILS\`. Connect never grants it. Removing the email strips admin from every token at once. Admin tokens still act as the account for ordinary \`/v1\` calls. They last at most 7 days (default 1 day) and cannot be never. Their hint inserts \`admin\` after the token prefix, then an ellipsis and the last four. \`GET /v1/admin/health\` is the read-only snapshot (quota used versus the cap, expired objects awaiting purge, stale purge claims). \`GET /v1/admin/audit\` lists recorded admin actions (who, which token, what, filters, counts, when). Needs an admin token. Never returns published bytes or secrets.
...
- Operators: \`GET /v1/admin/health\` is the read-only snapshot. \`POST /v1/admin/cleanup\` is the same preview/confirm/cap, host-wide, with \`owner\` (handle) and \`last_read_before\`. \`set_ttl\` without \`ttl\` is 7d. \`expire\` on anyone else's content is \`400 expire_not_own\`. Needs an admin token. Recorded at \`GET /v1/admin/audit\`. Never returns published bytes or secrets.
