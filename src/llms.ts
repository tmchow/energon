...
- An admin-scoped token is minted only at ${origin}/tokens by someone listed in \`ADMIN_EMAILS\`. The connect flow never grants it. Admin tokens live at most 7 days (default 1 day; never is not offered) and look like \`${id.tokenPrefix}adm…\`. \`GET /v1/whoami\` \`admin\` is true only while that owner is still on the list. Ordinary \`/v1\` calls still act as the account.
