...
- [Admin](${origin}/admin): operators listed on \`ADMIN_EMAILS\`. See a newer release, retire unused work, check storage health, and list or revoke tokens across accounts; every action is recorded. Signed-in humans only.
...
- \`GET /v1/sites\` and \`GET /v1/files\` list only what you created or last wrote. \`?scope=created|edited|involved\`, \`?q=\`, \`?created_by=\`, \`?expires=never\` or \`?expires_before=<iso>\` or \`?expires_within=24h|7d\` (windows computed at request time), \`?updated_before=<iso>\`, \`?min_size=<bytes or 500mb>\`, \`?sort=updated|name|size|age|last_read\`, \`?changed_since_read=1\`, \`?limit=\`, \`?cursor=\` (always intersected with your involvement — you cannot dump someone else's catalog). Responses include \`total\` and \`next_cursor\`. Search runs over the full involved set; pages are keyset cursors. Malformed filters are ignored, not 400. \`sort=last_read\` is never-read first, then oldest stamp. \`changed_since_read=1\` is \`updated_at\` after \`last_read_at\` (never-read counts as changed); it is a list filter, not a cleanup target. Every item carries \`last_read_at\`: when the Worker last served its bytes (public URL or \`/v1\`), \`null\` if never. It is a floor, not a view count: writes are throttled to about one per hour, and public reads answered from the edge cache do not reach the Worker. The edge cache holds public responses for at most a day, so the stamp lags real reads by at most about a day plus the hourly throttle.
