...
- Who can write: this instance default is \`${policy.writePolicy}\` (\`owner\` = only the creator, \`instance\` = any token on this host). Set \`write_policy\` on create to override. \`PATCH write_policy\` is creator-only. Anyone with a token can still read via \`/v1\`.
