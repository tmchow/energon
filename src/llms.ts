...
- An admin token is opt-in at mint on /tokens by someone on \`ADMIN_EMAILS\`. Connect never grants it. Removing the email strips admin from every token at once. Admin tokens still act as the account for ordinary \`/v1\` calls. They last at most 7 days (default 1 day) and cannot be never. Their hint inserts \`admin\` after the token prefix, then an ellipsis and the last four. \`GET /v1/admin/audit\` lists recorded admin actions (who, which token, what, filters, counts, when). Needs an admin token. Never returns published bytes or secrets.
