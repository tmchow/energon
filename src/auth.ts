...
export function helpBody(origin: string, env?: Env): unknown {
  ...
  return {
    ...
    sop: [
      ...
      `Make a copy: POST /v1/sites {"slug":"new-slug","duplicate_from":"existing-slug"} or POST /v1/files {"duplicate_from":"id"} (optional filename). You become created_by. write_policy is this Energon's default. Fresh TTL. Password is not copied. Anyone who can read via /v1 can duplicate. If you already have replacement bytes this turn, POST/PUT those instead.`,
    ...
    routes: {
      ...
      "POST /v1/sites": '{ "slug", "overwrite": false, "password"?: string, "write_password"?: string, "ttl"?: string, "write_policy"?: "owner"|"org", "duplicate_from"?: slug }',
      "PATCH /v1/sites/{slug}": '{ "password"?: string, "write_password"?: string, "ttl"?: string, "write_policy"?: "owner"|"org" } — empty password or write_password clears. ttl resets expiry from now. write_policy and write_password are creator-only.',
