...
export function helpBody(origin: string, env?: Env): unknown {
  ...
  return {
    ...
    sop: [
      ...
      `Clean up in bulk with POST /v1/cleanup: target ids or the list filters (expires=never, updated_before, min_size, q, created_by), action delete, set_ttl (with ttl), or expire (30m grace). Without confirm it is a dry run. Show the human the preview (matched, eligible, skipped, bytes, sample), then resend the same body with its confirm to execute. GET /v1/sites and GET /v1/files take the same filters plus sort=size|age to find candidates first. GET /v1/export first if they need a copy of what they own; cleanup {} is involvement, which is wider than ownership.`,
    ...
    routes: {
      ...
      "GET /v1/export":
        "zip of every site and loose file you own (owner_id), plus manifest.json (ids, names, public URLs, sizes, expiry, write_policy). Not involvement. Same zip caps as a site export; over the cap, export per site. Secrets are omitted. Empty ownership is 400 empty_export.",
      ...
      "GET /v1/admin/health":
        "admin token. Quota used and cap, catalog SUM(size), expired objects awaiting purge, stale purge claims, locked share gates, catalog counts. Never returns published bytes or secrets.",
      "POST /v1/admin/quota/recompute":
        "admin token. Sets platform_quota.used from SUM(size) over site_files and loose_files. Returns used_before and used_after. Recorded in GET /v1/admin/audit. Never returns published bytes or secrets.",
      "POST /v1/admin/sweep":
        "admin token. Runs one expiry sweep (same cap as the five-minute cron). Returns swept site and file counts and expired_remaining. Remaining > 0 means run again. Recorded in GET /v1/admin/audit. Never returns published bytes or secrets.",
      "POST /v1/admin/gates/unlock":
        'admin token. { "scope" } deletes that row from gate_attempts. Returns unlocked true when a row was removed. Recorded in GET /v1/admin/audit. Never returns published bytes or secrets.',
      "GET /v1/admin/tokens":
