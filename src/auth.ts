...
export function helpBody(origin: string, env?: Env): unknown {
  ...
  return {
    ...
    sop: [
      ...
      `An admin token is opt-in at mint on /tokens by someone on ADMIN_EMAILS. Connect never grants it. whoami.admin is true only while that token's scope is admin and the owner is still on the list. Removing the email strips admin from every token at once. Admin tokens still act as the account for ordinary /v1 calls. They last at most 7 days (default 1 day) and cannot be never. Their hint is ${id.tokenPrefix}admin… plus the last four. GET /v1/admin/audit lists recorded admin actions (who, which token, what, filters, counts, when). Needs an admin token.`,
      ...
      `Each site and file carries last_read_at in list and detail bodies: when the Worker last served its bytes over either path, null if never. It is a floor, not a view count: writes are throttled to about one per hour, and public reads answered from the edge cache never reach the Worker. Treat it as "read at least this recently", never as "unread since".`,
