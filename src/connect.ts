...
export async function connectResponse(env: Env, actor: Actor, id: string): Promise<Response> {
  ...
  return new Response(uiPage("Connect your agent — Energon", {
    page: "connect", data: { email: actor.email, host: new URL(publicOrigin(env)).host, connection: { id: connection.id, label: connection.label, expires_at: connection.expires_at }, token_policy: tokenPolicyPublic(tokenPolicy(env), publicOrigin(env)) }, footer: instanceFooter(env),
  }), {
