...
- **Catalog scan is padlock plus pencil, not a compound mark and not a jargon chip.** View password: the existing padlock, lit when that hash is set. Update password: a filled pencil (wide body, tip, eraser), lit when that hash is set. Both secrets: both icons, in conjunction. Draw them at **36px minimum**. Do not use a lock-with-corner-pencil; at catalog size it reads as a nail. Do not use a `write password` or `shared write` chip as the scan.
...
- `test/pages.spec.ts` disclosure, dialog unchanged/replace/remove, padlock on only for share password, filled pencil on for write password, both on when both set, icons ≥36px, Last writer stays the account, via copy, site write-password copy names homepage control
- `check:ui`
