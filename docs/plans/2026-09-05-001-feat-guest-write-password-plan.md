...
- **Objective:** An instance member can let someone outside the host replace one published loose file, or one existing path in a published site, without Access, token minting, or `/connect`. The outside agent learns the protocol from the content host.
...
- **Replace existing bytes only.** Guest site PUT 404s on a missing path. No new paths, no zip, no DELETE, no create-site. Loose-file PUT cannot rename.
...
- R8. PUT status matrix:
  ...
  - object or path missing → `404` JSON (not HTML)
  - expired → `410`
  - oversize / storage cap → `413` (same caps as token PUT)
  - success → `200` replace (no `api_url`, no `hub`)
...
- R11. Content origin `PUT` on `/{handle}/f/{id}/{filename}` and `/{handle}/s/{slug}/{path}` when the write header matches. Loose-file PUT requires handle, id, and filename to match the row (no 302). Site PUT replaces an existing path only. `PUT` to a directory URL (`…/s/{slug}/`) is `400` JSON naming the resolved index path if one exists. Same file-byte and platform-byte caps as token PUT. Same 410 for expired or purge-claimed objects.
...
- R27. verify-energon: new recipe plus updates to `share-password.md` and hub-catalog handles this change moves (`#stage-password` disclosure, dialog title, both fields, lock `on` for write-only). Proofs: guest PUT without `Authorization`; read header cannot PUT; cookie cannot PUT; identical phrases still header-bound; `write_policy=owner` guest PUT succeeds; creator-only set; clear write password then PUT 405; 405 when unset (no write header named); content vs hub `/llms.txt` when origins differ, guest section present when they do not; catalog still shows the object after guest PUT.

### Success Criteria

- An outside agent given the public URL and a write password can replace an existing file or site path without a token.
...
- Zip import / DELETE / new site paths via write password
...
- `test/api.spec.ts` status matrix, header-bound identical phrases, cookie refused, owner-policy guest PUT, creator-only set, duplicate drops write hash
...
**Act on (folded).** Guest PUT is not `putLooseFromRequest`. `WriteAuthority` not a fake `Actor`. Bytes-only. Replace existing paths only. Header-bound even when phrases match. Content `/llms.txt` is a Host branch of an existing public route, not a new 404 exception. Access is not why `/v1` is closed. Creator-only set. Hash in the claim predicate. Split rate-limit scopes. Do not put a sentinel in `last_written_by`. 405 when unset. No cacheable `Allow: PUT`. Purge on write-password PATCH. No CORS for PUT. Form does not take the write secret. Dialog omit vs clear per field. skill-render asserts constants. Single-origin `/llms.txt` rule. OpenAPI still lists every `ApiError` code. Verify map handles this change moves.

**Dismissed.** Concurrent gate burst rewrite (pre-existing sequential limiter, not this feature). PBKDF2 for write hashes (distinct prefix is the slice; share passwords already SHA-256). Guest zip / path listing (narrow the objective to known paths). Full write-event table (instrument later). Public HEAD as a vehicle for `Allow: PUT` (HEAD is unscoped; do not add it to advertise write).
