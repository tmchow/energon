...
- **Write implies read.** The write header unlocks GET. The HTML gate form accepts the write password for reading when a share password is also set. The cookie is still derived from the share hash. The share-password header does not accept the write phrase.
- **Write proof is the write header only.** `energon_gate` never authorizes PUT or DELETE, including a cookie minted after unlocking the gate with the write phrase.
- **Guess throttling is mitigation, not prevention.** Keep `GATE_MAX_FAILS = 20` and `GATE_WINDOW_MS = 15 minutes`. A leaked correct phrase cannot be stopped except by rotate or clear. Per-object scopes cap guesses globally; per-IP scopes limit spraying; write and read scopes stay separate (`obj:`/`ip:` vs `wobj:`/`wip:`).
- **Pay Shape A now.** Guest write stays parallel mutators in `src/guest-write.ts`. Do not extract shared `commit*Bytes` in this change.
...
- **Catalog password slot is one mark, never lock beside pencil.** View password only: the existing padlock. Write password present (write-only or view+write): the agreed diagonal lockup — lock northwest, filled pencil (wide body, tip, eraser) southeast, one composed mark. Do not place a padlock and a pencil as horizontal siblings. Do not fuse a tiny pencil onto the lock body. Do not use a `write password` or `shared write` chip as the scan. Org-write exception is a third family and may sit next to the password mark. People are overlapping (front person complete, companion peeking). Two equal rings over a connected double-hump are out (reads as a face). Off = the same people plus an X badge; badge circle stroke is thinner than the people (~1.15 vs 1.75). No hairline tile around the marks — boxed 36px glyphs overwhelmed the catalog. Marks are unboxed 28px. Hover (`title` and `aria-label`) names the mark: View password, Write password, Org can write, Org cannot write. Unset view-password control (today’s prototype lock on every row) is Set view password.
...
- R3. `/v1` GET never returns the secret. Set responses echo it once via `secretJson` (`no-store, private`). Catalog rows expose `write_password_protected: boolean`. Hub `GET /account/sites/{slug}` and `GET /account/files/{id}` return stored phrases to the signed-in human (`secretJson`). Hash-only legacy rows return `password: null` with `password_protected: true`.
...
- R23. Catalog password slot is one unboxed 28px glyph. View-password hash set and write-password hash unset: existing padlock. Write-password hash set (alone or with a view password): the diagonal lockup (lock NW, filled pencil SE). Write-only shows that lockup, not a lone pencil and not a padlock beside a pencil. Neither hash: no password mark. Hover names the mark (`View password` / `Write password`). One Link access dialog from that control (same dialog, two fields). Each door has Off/On (`#pw-dlg-share-door`, `#pw-dlg-write-door`). Phrase fields show when On, with generate and copy. Off and Save removes that door. Do not empty a field to remove. Do not use Keep/Replace/Remove. Do not place padlock and pencil as horizontal siblings. Do not fuse a pencil onto the lock body. Do not draw a hairline tile around the mark. Do not use a `password` / `write password` / `shared write` / `writable` text chip as the scan. Copy in the dialog: a file write password replaces that file only. A site write password is full control of served bytes, including replacing `index.html`. Instance tokens still follow write policy. Write password is for someone not on this host. Non-creators do not get the write-password fields. Catalog expiry is its own column and is blank when the work does not expire. Do not show Created by.
- R24. Gate page: the HTML form accepts the share password or the write password for reading. Cookie is still derived from the share hash. The share-password header still checks the share hash only. Cookie and form never authorize PUT or DELETE. DESIGN.md: a lock describes a share password; the NW-lock / SE-pencil lockup is the write-password mark. CONCEPTS.md gains Write Password. STRATEGY.md "shared writes happen through in-place replacement under write policy" needs a clause for the write-password door. Optional third catalog mark (org-write exception vs instance default) is overlapping people (front + peeking). On = those people (org can write). Off = the same people plus an X badge; badge circle stroke is thinner than the people (~1.15 vs 1.75) so the X stays open. No lone person silhouette. No pencil on the people mark. Thin slash through the group is out. No hairline tile. Password mark and people mark may appear on the same row. Marks are unboxed 28px. Hover names the people mark (`Org can write` / `Org cannot write`).
...
- A human given only the write password can unlock the HTML gate for reading when a share password is also set; that cookie still cannot PUT or DELETE.
...
- KTD6. Set/clear `write_password` uses `assertCanSetWritePolicy`, including `overwrite: true`. Duplicate never copies the write hash or secret. Catalog JSON exposes `write_password_protected` boolean only. Hub account GET returns recoverable phrases; `/v1` GET does not.
...
- KTD9. Hub stage: URL, expiration, Who can write, Publish stay visible. Closed `<details id="stage-access">` Link access holds share password and write password; open when either is non-empty. One `#pw-dlg` retitled Link access. Each door has Off/On (`#pw-dlg-share-door`, `#pw-dlg-write-door`); stored phrases in `PasswordField` (generate + copy) show when On. Off and Save clears that door. Catalog: view-only hash → lock; write hash set → lockup; neither → no password mark; no Password chip; Expires column blank when unset; no Created by column. More menu still offers Set password. Drop the Hub Catalog marks legend card once live rows can show lockup. Org people marks stay as today.
...
Stage: `#stage-access` closed disclosure. Dialog: Off/On per door; phrase fields when On. Catalog: lock / lockup / none per KTD9. Via copy under last writer. Flash two secrets. Site write-password note names homepage control. File note: replaces that file only.
...
- A viewer with only the share password cannot PUT or DELETE. A gate cookie cannot PUT or DELETE. The HTML gate accepts the write password for reading.
...
None blocking. Catalog mark size is 28px unboxed. Hover names the mark. Neither hash: no password mark (R23); More menu still Set password. Deferred: whether to later extract Shape B shared commits once guest and actor paths have both been green in production. This change pays Shape A; do not extract now.

### Product amendment (2026-09-07)

Gate form accepts the write password for **reading** when a share password is also set. Cookie is still derived from the share hash and still never PUT/DELETE. Share-password header still does not accept the write phrase. Guess throttling stays 20 fails / 15 minutes as mitigation, not prevention. Write and read scopes stay separate.
