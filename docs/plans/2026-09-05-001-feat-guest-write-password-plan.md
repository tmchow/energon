...
- **Catalog scan is padlock plus pencil, not a compound mark and not a jargon chip.** View password: the existing padlock, lit when that hash is set. Update password: a filled pencil (wide body, tip, eraser), lit when that hash is set. Both secrets: both icons, in conjunction. Draw them at **24px minimum, preferably larger** than today's 19px catalog glyph. Do not use a lock-with-corner-pencil below 24px; it reads as a nail. Do not use a `write password` or `shared write` chip as the scan.
...
- R23. Catalog scan uses two sibling icons at **24px minimum** (prefer larger than `--icon-size` 19px): the existing padlock for the view password, a filled pencil for the update password. Padlock `on` only when the share-password hash is set. Pencil `on` only when the write-password hash is set. Both on when both hashes are set. Write-only lights the pencil, not the padlock. One Link access dialog from those controls (same dialog, two fields). Each secret is `unchanged | replace | remove`. Empty box does not clear the other secret. Do not encode this with a 16px/19px compound lock+pencil, and do not use a `write password` / `shared write` / `writable` text chip as the scan. Optional existing `password` chip may stay for the view door or drop once the padlock is enough. Copy in the dialog: a file write password replaces that file only. A site write password is full control of served bytes, including replacing `index.html`. Instance tokens still follow write policy. Write password is for someone not on this host. Non-creators do not get the write-password fields.
- R24. Gate page stays a share-password unlock. Do not accept the write secret in the form. DESIGN.md: a lock describes a share password; the filled pencil is the update-password mark; both render at ≥24px. CONCEPTS.md gains Write Password. STRATEGY.md "shared writes happen through in-place replacement under write policy" needs a clause for the write-password door.
...
- `test/pages.spec.ts` disclosure, dialog unchanged/replace/remove, padlock on only for share password, filled pencil on for write password, both on when both set, icons ≥24px, Last writer stays the account, via copy, site write-password copy names homepage control
- `check:ui`
...
**Session-settled after pass 2.** Last writer stays the last account; never `guest`. Via copy is "Updated via shared write". Column name is `written_via`. Catalog scan is padlock (view password) plus filled pencil (update password), both ≥24px, shown in conjunction when both secrets are set. No `shared write` chip. Content-host `/v1/help` is 404 JSON that names `GET /llms.txt` on this host. Guest loose-file PUT reuses the stored `Content-Type`.

## Open questions

None that block implementation. Exact pixel size above 24px can be tuned in the UI PR as long as the pair stays larger than today's 19px catalog glyph.
