...
- **Lock means share password.** Catalog lock `on` only when a share-password hash is set. Write-only (public read + guest write) does not light the lock. A second badge `shared write` marks the write door. Do not reuse `password` or `writable`.
...
- R23. Catalog lock opens one Link access dialog. Each secret is `unchanged | replace | remove`. Empty box does not clear the other secret. Lock `on` only when the share-password hash is set (read gate). Write-only does not light the lock. Badge `password` if share-passworded. Second badge `shared write` if write-passworded (not `password`, not `writable`, not `edit`, not a role). Copy: a file write password replaces that file only. A site write password is full control of served bytes, including replacing `index.html`. Instance tokens still follow write policy. Write password is for someone not on this host. Non-creators do not get the write-password fields.
- R24. Gate page stays a share-password unlock. Do not accept the write secret in the form. DESIGN.md stays: a lock describes a share password. CONCEPTS.md gains Write Password. STRATEGY.md "shared writes happen through in-place replacement under write policy" needs a clause for the write-password door.
...
- `test/pages.spec.ts` disclosure, dialog unchanged/replace/remove, `password` badge vs `shared write` badge, lock on only for share password, Last writer stays the account, via copy, site write-password copy names homepage control
- `check:ui`
...
**Session-settled after pass 2.** Last writer stays the last account; never `guest`. Via copy is "Updated via shared write". Column name is `written_via`. Catalog lock `on` only for share password. Badge for the write door is `shared write`. Content-host `/v1/help` is 404 JSON that names `GET /llms.txt` on this host. Guest loose-file PUT reuses the stored `Content-Type`.

## Open questions

None that block implementation. Copy nits during implement (`shared write` vs a shorter chip) may be adjusted in the UI PR without reopening protocol.
