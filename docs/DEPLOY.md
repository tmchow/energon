...
`instance-skill.json`, `templates/`, and `version.txt` are the sources. Generated plugin and marketplace manifests take `version` from `version.txt` (the Energon release). There is no per-instance plugin version. Before initialization, `npm run skill:render` and its `--check` form validate templates without generating files. After initialization, render refreshes the package and catalogs; `--check` fails if they drift, including when generated versions lag `version.txt`. Keep the Worker identity variables aligned with the manifest. Do not hand-edit generated files or put the publish skill in `.agents/skills` or `.claude/skills`.

Updating this repository or deploying the Worker does not update plugins already installed on agent machines. Clients refresh from the marketplace as [Refresh an installed plugin](../INSTALL.md#refresh-an-installed-plugin) describes.
