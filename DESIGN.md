...
| Catalog and usage | `Table`, `Badge`, `CatalogScan`, `Metric`, `ProgressBar` | Unboxed 28px access marks; row action glyphs fill the 36px control so their ink matches those marks. Hover names the mark. Preserve scanning, units, and the meaning of state |
