...
| Markdown | Read the author's document | Readable code, tables, and diagrams in both color schemes; source via curl or `?raw=1` |
...
The app page measure is `--measure-page` (74rem), Markdown `--measure-md` (46rem), and introductory copy `--measure-lede` (36rem). Markdown prose stays on `--measure-md`. Mermaid and wide tables may sit slightly wider (`--measure-md + 6rem`) without stretching the page. Mermaid is a shrink-to-fit preview (intrinsic SVG, CSS scale, a floor on height). Every mermaid figure uses that preview, not only the first or a wide one. Paint mermaid from Energon surfaces rather than Mermaid's stock themes: the figure is a tinted ground (paper-2 in light, ink-2 in dark), nodes are an elevated fill with `--fg-3` hairlines, clusters sit one step further from the canvas, and edge labels are elevated chips with a hairline — not a wash the same color as the page. Matching the SVG canvas to the page without lifting those fills collapses node, cluster, and edge contrast. A wide table may scroll horizontally inside that figure. Both open the same pannable overlay (opaque lightbox on desktop, fullscreen on a phone) instead of clipping with no way out. The overlay uses the markdown canvas (`--bg-canvas`), not a see-through dim. Narrow tables stay in the prose measure with no extra chrome. Controls use `--radius-1` (4px), cards and dialogs `--radius-2` (8px), and sheets `--radius-3` (12px). Pills are for the existing bar treatment, not a default shape for everything.
