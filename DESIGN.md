...
The app page measure is `--measure-page` (74rem), Markdown `--measure-md` (46rem), and introductory copy `--measure-lede` (36rem). Markdown prose stays on `--measure-md`. Mermaid keeps its intrinsic size and uses the viewport content width; if a diagram is still wider, that figure pans horizontally instead of clipping or stretching the page. Controls use `--radius-1` (4px), cards and dialogs `--radius-2` (8px), and sheets `--radius-3` (12px). Pills are for the existing bar treatment, not a default shape for everything.
