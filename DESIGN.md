...
The app page measure is `--measure-page` (74rem), Markdown `--measure-md` (46rem), and introductory copy `--measure-lede` (36rem). Mermaid diagrams keep their intrinsic size inside that measure; the article scrolls horizontally instead of shrinking a wide graph until it is unreadable. Controls use `--radius-1` (4px), cards and dialogs `--radius-2` (8px), and sheets `--radius-3` (12px). Pills are for the existing bar treatment, not a default shape for everything.
