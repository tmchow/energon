...
### Token Prefix
The marker at the beginning of an API Token that identifies which token format this Energon accepts. It is part of this Energon's identity and must remain consistent when tokens are minted, authenticated, masked, or described to agents.
...
### This Energon's identity
The runtime configuration that tells agents which Energon they are using and how to interact with it. It includes the origin, skill and marketplace coordinates, token environment variable, and Token Prefix.

Identity is resolved from deployment configuration with project defaults as fallbacks. Authentication and credential presentation use the same identity so this Energon cannot advertise or issue credentials in a format it will reject.

Host is a locative for the place (this origin, vs someone not on this host). It is not the name of the product unit and not “host identity.”

## Relationships

- A Site contains Site files; a Loose file is published outside any Site.
- This Energon's identity defines the Token Prefix used by an API Token.
