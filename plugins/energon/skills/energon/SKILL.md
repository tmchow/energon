...
1. Look for env `ENERGON_TOKEN`. If it is missing, **stop and tell the human** to open https://energon.example.com/tokens, mint a key, and export it. Do **not** invent a token. Do **not** reuse a token you saw in chat history unless they just pasted it.
