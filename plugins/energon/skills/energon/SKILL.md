...
1. Look for env `ENERGON_TOKEN`. If it is missing, read https://energon.example.com/auth.md and request a human-approved connection, or ask the human to mint and export a token at https://energon.example.com/tokens. Do not automate the human approval. Do **not** invent a token. Do **not** reuse a token you saw in chat history unless they just pasted it. A token that has expired (`401` with `error: token_expired`) is the same stop: it cannot be renewed; the human must approve a new connection or mint a replacement.
...
Auth on every `/v1` call except `/v1/help`, `/v1/health`, `/v1/openapi.json`, and the connection request/poll endpoints documented in `/auth.md`:
...
Stop using a rejected or expired token. Read https://energon.example.com/auth.md for the human-approved connection flow: show the returned link and code, wait for approval, and poll with the private `poll_token`. Store the delivered token without printing it. Denial is terminal; a lost delivery requires human revocation before a new approval. This is not an OAuth flow.

Manual tokens are also supported. Tell the human:
...
Then retry with the **new** token. Do not scrape HTML for a token. Do not automate approval or mint through the human hub API; only the connection flow delivers a token after human approval.
